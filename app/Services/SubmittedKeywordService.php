...
class SubmittedKeywordService
{
    ...
    */
        ...
        if ($websiteData['supported_markets'] != null) {
            ...
            if (in_array($submission['market'], $supportedMarkets) || strtolower($submission['feed']) == 'iac') {
                ...
                $newCampaignData = [
                    ...
                    'buying_type' => $campaign['buying_type'],
                    'daily_budget' => 500,
                    'status' => $this->facebookCampaign->determineStatus($campaign['status'], $targetAccount),
                    'special_ad_categories' => $campaign['special_ad_categories']
                ];
                ...
                foreach ($existingCampaignAdsets[1] as $existingAdSet) {      
                    ...
                    if (in_array('desktop',  $devicePlatforms)) {
                        $newAdsetTargeting['facebook_positions'] = ['feed', 'right_hand_column', 'marketplace'];
                    }
                                ...
                                if ($existingAdCreative[0] == false) {
                                    ...
                                    if ($newAdCreative[0] == false) { 
                                        ...
                                        $newAdData = [
                                            ...
                                            'status' => $this->facebookCampaign->determineStatus($existingAd->status, $targetAccount),
                ...
                if (count($campaignsToTrack) > 0) {
                    ...
                    foreach ($campaignsToTrack as $key => $data) {
                        ...
                        if ((string) $data['feed'] == 'iac' && $shouldQueueForDuplication === true) {
                            $cdService->create([
    ...
    public function deleteRowByBatchId(string $batchId) 
    {
        return SubmittedKeyword::where('batch_id', $batchId)->delete();
    }
