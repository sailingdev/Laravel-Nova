<template>
    <div class="w-full pt-5 pb-5">
        ...
        <h1 class="text-center p-4">Break Down By Date</h1>
    ...
    watch: {
        triggerReload(newVal, oldVal) {
            if (newVal > oldVal && this.typeTag != '') {
                ...
                this.$emit('dailyTotalsLoadingStatus', true)
            }
        }
    },
    computed: {
        dailyTotalsTableHeader() {
            return [
                'DATE', 'SPEND', 'REVENUE', 'PROFIT', 'ROI', 'CLICKS', 'RPC', 'CPA'
            ]
        },
    },
    methods: {
        ...
        loadWebsiteTotals() {
            ...
            .then(response => {  
                ...
                this.$emit('renderTrendMetrics', response.data.data.metrics)
            }).catch(error => {  
                this.errorResponse = error.response.data
            }).finally(() => {
                this.loading = false
                this.$emit('dailyTotalsLoadingStatus', false)
            })
        }
    }
